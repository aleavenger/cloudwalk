...
- optional external provider path rewrites only `summary` and `top_recommendation`, with sanitized fallback state on errors
...
- Optional external narrative: `EXTERNAL_AI_PROVIDER`, `EXTERNAL_AI_MODEL`, `EXTERNAL_AI_API_KEY`, `EXTERNAL_AI_TIMEOUT_SECONDS`
...
- Dashboard provisioning (`grafana/dashboard.json`) uses a default `now-5y` range so historical dataset-backed metrics render immediately.
- Metric visualizations are provisioned with Infinity backend parser plus `timeseries` query format and explicit timestamp/number columns to satisfy Grafana 11 panel rendering.
- Decision and evidence tables consume backend-provided auth-code display fields rather than raw JSON arrays.
- Dashboard layout is decision-first: snapshot, priority queue, forecast risk, evidence, formal alerts, rate trend, volume context, and first-login guidance.
...
5. Divergent behavior between compose mode (`api:8000`) and local-host mode (`127.0.0.1:8000`) for Grafana datasource URLs.
